...
switch($method) {
    case 'GET':
        ...
        $sql = "SELECT a.*, u.identificacion, u.nombres, u.apellidos, u.email, u.telefono, 
                   u.fecha_nacimiento, u.direccion, u.activo
    ...
    case 'POST':
        ...
        try {
            ...
            $stmt_user->bind_param("sssssssii", 
                $data['identificacion'],
                $data['nombres'],
                $data['apellidos'],
                $data['email'],
    ...
    case 'PUT':
        ...
        try {
            ...
            if(isset($data['password']) && !empty($data['password'])) {
                $password_hash = password_hash($data['password'], PASSWORD_DEFAULT);
                $sql_user = "UPDATE usuarios SET identificacion=?, nombres=?, apellidos=?, email=?, telefono=?, fecha_nacimiento=?, direccion=?, password_hash=? WHERE id=?";
                $stmt_user = $conn->prepare($sql_user);
                $stmt_user->bind_param("ssssssssi", 
                    $data['identificacion'], $data['nombres'], $data['apellidos'], $data['email'], 
                    $data['telefono'], $data['fecha_nacimiento'], $data['direccion'], $password_hash, $data['usuario_id']
                );
            } else {
                $sql_user = "UPDATE usuarios SET identificacion=?, nombres=?, apellidos=?, email=?, telefono=?, fecha_nacimiento=?, direccion=? WHERE id=?";
                $stmt_user = $conn->prepare($sql_user);
                $stmt_user->bind_param("sssssssi", 
                    $data['identificacion'], $data['nombres'], $data['apellidos'], $data['email'], 
                    $data['telefono'], $data['fecha_nacimiento'], $data['direccion'], $data['usuario_id']
                );
            }
            $stmt_user->execute();
